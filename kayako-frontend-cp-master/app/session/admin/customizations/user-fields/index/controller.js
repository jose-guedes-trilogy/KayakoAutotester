...
export default Controller.extend({
  ...
  // Actions
  actions: {
    transitionToNewUserFieldRoute() {
      this.transitionToRoute('session.admin.customizations.user-fields.select-type');
    },
    ...
    toggleEnabledStatus(field, e) {
      ...
      this.get('customFields').toggleEnabled(field).then(() => {
        ...
        if (field.get('isEnabled')) {
          notificationMessage = this.get('i18n').t('admin.userfields.enabled.success_message');
        } else {
          notificationMessage = this.get('i18n').t('admin.userfields.disabled.success_message');
        }
    ...
    deleteField(field, e) {
      ...
      .then(() => {
        let msg = this.get('i18n').t('admin.userfields.delete.success_message');
        this.get('notification').success(msg);
      });
    },

    editField(field) {
      this.transitionToRoute('session.admin.customizations.user-fields.edit', field.id);
    },
