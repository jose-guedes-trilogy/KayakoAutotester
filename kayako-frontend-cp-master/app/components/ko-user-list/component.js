...
const PAGE_LIMIT = config.userListPageSize;
...
export default Component.extend({
  ...
  didReceiveAttrs: diffAttrs('page', function(changedAttrs, ...args) {
    ...
    if (!changedAttrs || changedAttrs.page) {
      this.fetchUsersWithCompletePropositions();
    }
  }),

  //CP's
  users: reads('fetchUsers.lastSuccessful.value'),
  ...
  actions: {
    ...
    changePropositionOperator(definitionLabel, operator) {
      let currentPropositionOperators = this.get('currentPropositionOperators');

      currentPropositionOperators.set(definitionLabel, operator);
  ...
  //Tasks
  fetchUsers: task(function * (queryToRun) {
    const router = this.get('router');
    yield timeout(300);

    let results = yield this.get('store').query('user', queryToRun);
  ...
  fetchUsersWithCompletePropositions() {
    let predicateCollections = this.get('currentPredicateCollections');
    ...
    if (!lodash.isEqual(query, lastRunQuery)) {
      ...
      this.get('fetchUsers').perform(query);
    }
  }
});
