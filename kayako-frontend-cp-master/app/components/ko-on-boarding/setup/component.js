...
export default Component.extend({
  ...
  patternUrl: `${config.messengerAssetsUrl}1--dark.svg`,
  ...
  isDarkGradient: computed('activeGradient', function() {
    ...
    if (gradientValue) {
      ...
      let lightness1 = this._getColorDarkness(colors[0]);
      let lightness2 = this._getColorDarkness(colors[1]);
  ...
  connectTwitter(params) {
    ...
    this.get('store').createRecord('twitter-account-callback', {
      ...
      this.get('notification').add({
        type: 'success',
        title: this.get('i18n').t('onboarding.setup.popup.social.connect.twitterconnected'),
        autodismiss: true
      });
      this.setProperties({
        twitterConnected: true,
  ...
  saveAgents: task(function * (invitation) {
  ...
  saveTeam: task(function * () {
    const defaultTeam = yield this.get('store').findRecord('team', 1);
    defaultTeam.set('title', this.get('defaultTeamTitle'));
    yield defaultTeam.save();

    for(let team of this.get('newTeams')) {
      yield this.get('store').createRecord('team', {
        title: team.value
      }).save();
    }

    yield this.get('onBoardingStatus').updateStatus('team_added', {'user.team_added': '1'});
    this.set('teamSetup', 'done');
    this.onComplete();
  }),
  ...
  publishMessenger: task(function * () {
  ...
  actions: {
    ...
    importFacebookPages() {
      ...
      adapter.ajax(url, 'POST', {
        ...
        this.get('notification').add({
          type: 'success',
          title: this.get('i18n').t('onboarding.setup.popup.social.connect.facebookpagessuccess'),
          autodismiss: true
        });
