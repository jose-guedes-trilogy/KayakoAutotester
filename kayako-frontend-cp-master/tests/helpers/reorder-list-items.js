...
export function reorder(app, itemSelector, elementSelector, ...resultSelectors) {
  ...
  resultSelectors.forEach((selector, targetIndex) => {
    andThen(() => {
      ...
      let element = items.filter((index, element) => { return $(element).parent().find(elementSelector).text().trim() === selector; });
...
export default registerAsyncHelper('reorderListItems', reorder);
