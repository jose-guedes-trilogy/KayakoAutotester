...
moduleFor('service:error-handler/csrf-invalid-strategy', 'Unit | Service | error-handler/csrf-invalid-strategy', {
  integration: true,
  ...
  let subject = this.subject({
    ...
    session: Service.create({
      reportSessionExpiry(message) {
        assert.equal(message, 'Invalid/Missing CSRF. Called from app/services/error-handler/csrf-invalid-strategy.js');
      },
      clear() {
        assert.ok(true);
      }
