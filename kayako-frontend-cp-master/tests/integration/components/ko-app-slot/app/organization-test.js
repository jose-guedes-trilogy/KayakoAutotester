...
moduleForComponent('ko-app-slot/app', 'Integration | Component | ko-app-slot/app | organization page', {
  integration: true,
  beforeEach() {
...
test('Responds to get_data events for top level organization', function(assert) {
    ...
    port.onmessage = (e) => {
      if (e.data.ref === messageRef) {
        ...
        assert.equal(stableJSON(e.data.payload.data), stableJSON({
          id: 123,
          name: 'Foo Corp'
        }), 'replied with organization');
        done();
      }
    };

    // 1. Client.js triggers get_data event
    port.postMessage({ event: 'get_data', payload: { path: 'organization' }, ref: messageRef });
  };
});
  ...
  iframeWindow(this).bufferedOnMessage = (e) => {
    ...
    port.onmessage = (e) => {
      ...
      if (e.data.ref === subscribeMessageRef) {
        ...
        // 3. trigger a change
        this.get('organization').set('name', 'Another Org');
    ...
    port.postMessage({ event: 'add_event_listener', payload: { type: 'change', path: 'organization' }, ref: subscribeMessageRef });

  };
});

test('Changing the organization re-initializes the iframe', function(assert) {
  this.set('app', DUMMY_APP);
  this.set('slot', DUMMY_SLOT);
  ...
  this.set('organization', EmberObject.create({ id: 123 }));
  ...
  iframeWindow(this).bufferedOnMessage = (e) => {
    ...
    this.set('organization', EmberObject.create({ id: 234 }));
...
test('Changing the organization tears down any listeners', function(assert) {
  ...
  iframeWindow(this).bufferedOnMessage = (e) => {
    ...
    port.postMessage({ event: 'add_event_listener', payload: { type: 'change', path: 'organization' }, ref: subscriptionRef });
  };
