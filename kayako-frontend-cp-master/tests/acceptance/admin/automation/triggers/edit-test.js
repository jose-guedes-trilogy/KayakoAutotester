...
app('Acceptance | admin/automation/triggers/edit', {
  beforeEach() {
...
test('Exit having pending changes asks for confirmation', function(assert) {
  assert.expect(3);
...
test('when any is selected for channel, null is sent for channel and event', function(assert) {
  assert.expect(3);

  server.post('/api/v1/triggers', (schema, req) => {
    let data = JSON.parse(req.requestBody);
    assert.equal(data.channel, null);
    assert.equal(data.event, null);
    let newTrigger = server.create('trigger', data);
    return {
      status: 201,
      data: newTrigger,
      resource: 'trigger',
...
test('when any is selected for event null is sent', function(assert) {
  assert.expect(3);
