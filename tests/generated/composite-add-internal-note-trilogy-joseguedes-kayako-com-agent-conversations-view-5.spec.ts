...
if (!queueMode) {
  test.describe("Switch to internal note mode, add text, and send. (https://trilogy-joseguedes.kayako.com/agent/conversations/view/5)", () => {
    test('composite-add-internal-note-trilogy-joseguedes-kayako-com-agent-conversations-view-5', async ({ authenticatedPage: page }) => {
      await runFlow(page);
    });
  });
}