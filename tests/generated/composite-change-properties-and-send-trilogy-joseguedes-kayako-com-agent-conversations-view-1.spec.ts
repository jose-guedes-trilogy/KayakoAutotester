...
export async function runFlow(page: Page): Promise<void> {
    await page.goto("https://trilogy-joseguedes.kayako.com/agent/conversations/view/1");
...
if (!queueMode) {
  test.describe("Update ticket properties (status/custom fields) and send a reply. (https://trilogy-joseguedes.kayako.com/agent/conversations/view/1)", () => {
    test('composite-change-properties-and-send-trilogy-joseguedes-kayako-com-agent-conversations-view-1', async ({ authenticatedPage: page }) => {
      await runFlow(page);
    });
  });
}