...
if (!queueMode) {
  test.describe("Apply macro, finalize reply, update status, and save assignee. (https://trilogy-joseguedes.kayako.com/agent/conversations/view/5)", () => {
    test('composite-apply-macro-and-close-trilogy-joseguedes-kayako-com-agent-conversations-view-5', async ({ authenticatedPage: page }) => {
      await runFlow(page);
    });
  });
}