...
export async function runFlow(page: Page): Promise<void> {
    ...
    await expectVisible(page, 'users', 'results');
    await expectVisible(page, 'users', 'firstRow');
}

if (!queueMode) {
  test.describe("Ensure users results table renders with at least one row", () => {
    test('users-results-visible', async ({ authenticatedPage: page }) => {
      await runFlow(page);
    });
  });
}