...
export async function runFlow(page: Page): Promise<void> {
    await createNewTicket(page, env.KAYAKO_USERNAME, "[AUTOTEST] Ticket full properties", "[AUTOTEST] Ticket full properties body", 'reply');
    await setStatus(page, "Open");
    await setCustomField(page, { type: "dropdown", label: "Priority", value: env.DEFAULT_PRIORITY, path: undefined } as any);
    await setCustomField(page, { type: "dropdown", label: "Type", value: undefined, path: undefined } as any);
    await setCustomField(page, { type: "dropdown", label: "Form", value: undefined, path: undefined } as any);
    await addTags(page, ["autotest-props"]);
    await expectTagsContain(page, ["autotest-props"]);
    ...
    await (await import('../../selectors')).switchAssigneeTeamAndSave(page, ["General", "VIP Account Team"]);
    await click(page, 'assign', 'updatePropertiesButton');
    await expectStatusLabel(page, "Open");
    await page.waitForTimeout(2000);
}

if (!queueMode) {
  test.describe("Create a brand new ticket and fill all ticket properties (status, tags, and custom fields), then verify them", () => {
    test('ticket-properties-complete', async ({ authenticatedPage: page }) => {
      await runFlow(page);
    });
  });
}