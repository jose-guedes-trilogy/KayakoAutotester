...
export async function runFlow(page: Page): Promise<void> {
    await createNewTicket(page, env.KAYAKO_USERNAME, "[AUTOTEST] Ticket New→Open", "[AUTOTEST] Ticket New to Open body", 'reply');
...
if (!queueMode) {
  test.describe("Create a new ticket and change its status from New to Open via the Status field", () => {
    test('ticket-status-new-to-open', async ({ authenticatedPage: page }) => {
      await runFlow(page);
    });
  });
}