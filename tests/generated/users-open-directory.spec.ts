...
export async function runFlow(page: Page): Promise<void> {
    ...
    await expectVisible(page, 'users', 'container');
    await expectVisible(page, 'users', 'sidebar');
    await expectVisible(page, 'users', 'resultsHeading');
}

if (!queueMode) {
  test.describe("Open the Users directory and verify core layout", () => {
    test('users-open-directory', async ({ authenticatedPage: page }) => {
      await runFlow(page);
    });
  });
}